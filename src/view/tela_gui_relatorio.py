...
class TelaRelatorio:
    ...
    def exibir_mensagem(self, mensagem: str):
        sg.popup(mensagem, title="Mensagem", background_color=self.__background_color, text_color=self.__text_color)
    ...
    def exibir_ultima_compra(self, venda):
        ...
        layout = [
            [sg.Text(f"Última Compra de {venda['cliente']['nome']}", font=("Courier", 18, "bold"),
                     text_color=self.__text_color, background_color=self.__background_color)],
            [sg.Text("Vendedor:", font=("Arial", 12), background_color=self.__background_color, text_color="white"),
             sg.Text(venda['vendedor']['nome'], font=("Arial", 12), background_color=self.__background_color, text_color="white")],
            [sg.Text("Data da venda:", font=("Arial", 12), background_color=self.__background_color, text_color="white"),
             sg.Text(venda['data_venda'], font=("Arial", 12), background_color=self.__background_color, text_color="white")],
        ...
        window = sg.Window("Detalhes da Última Compra", layout, background_color=self.__background_color, finalize=True)
        while True:
            event, _ = window.read()
    ...
    def exibir_ultima_venda(self, venda):
        ...
        window = sg.Window("Detalhes da Última Venda", layout, background_color=self.__background_color, finalize=True)
        while True:
            event, _ = window.read()
            if event in [sg.WINDOW_CLOSED, "OK"]:
                break
    ...
    def mostrar_relatorio_por_tipo_cliente(self, relatorio_por_tipo: dict):
        ...
        window = sg.Window("Relatório por Tipo de Cliente", layout, background_color=self.__background_color, finalize=True)
        while True:
            event, _ = window.read()
            if event in [sg.WINDOW_CLOSED, "OK"]:
                break
    ...
    def selecionar_dia_relatorio(self, dias_disponiveis: List[str]) -> Optional[datetime]:
        ...
        window = sg.Window("Relatório por Dia", layout, background_color=self.__background_color, finalize=True)
