...
class TelaVendas(AbstractTelaCadastro):
    ...
    def obter_dados_venda(self, clientes: List[Dict], vendedores: List[Dict], produtos: List[Dict]) -> Dict:
        layout_cliente_vendedor = [
            ...
            [sg.Text("Ou digite o CPF para buscar:", text_color="white", background_color="#2C2F36")],
            [
                sg.InputText(key="cpf_vendedor",
                             size=(25, 1), background_color="#3E4349", text_color="white"),
        ...
        layout_produtos_selecionados = [
            ...
            [sg.Listbox(values=[f"{produto['nome']} - "
                                f"Preço: R${produto['preco']:.2f}" for produto in produtos],
                        select_mode=sg.LISTBOX_SELECT_MODE_MULTIPLE, size=(50, 10),
        ...
        while True:
            ...
            if event in (sg.WIN_CLOSED, "Cancelar"):
                if any(values[key] for key in
                       ["cpf_cliente", "cpf_vendedor", "cliente_selecionado", "vendedor_selecionado"]):
                    confirmar = sg.popup_yes_no(
                        "Há dados preenchidos. Deseja realmente sair?\nOs dados não salvos serão perdidos.",
            ...
            if event == "buscar_cliente":
                ...
                cliente_encontrado = next((f"CPF: {c['cpf']} - "
                                           f"Nome: {c['nome']}" for c in clientes if c["cpf"] == cpf),
                                          None)
                if cliente_encontrado:
                    janela["cliente_selecionado"].update(cliente_encontrado)
                else:
                    sg.popup("Cliente não encontrado!",
                             title="Erro", text_color="red", background_color="#2C2F36")
                ...
                if not cliente:
                    ...
                    continue
                if not vendedor:
                    sg.popup("Selecione um vendedor!",
                             title="Erro", text_color="red", background_color="#2C2F36")
                    continue
        ...
        layout_quantidades = [
            [sg.Text("Informe as Quantidades:", font=("Courier", 20),
        ...
        janela_quantidade = sg.Window("Selecionar Quantidades",
                                      layout_quantidades, background_color="#2C2F36",
                                      modal=True)
        ...
        while True:
            ...
            if event in (sg.WIN_CLOSED, "Cancelar"):
                ...
                try:
                    ...
                    sg.popup("Informe quantidades válidas para todos os produtos.",
                             title="Erro", text_color="red",
                             background_color="#2C2F36")
    ...
    def seleciona_vendas(self, vendas: List[Dict]) -> int:
        layout = [
            ...
            [sg.Listbox(values=[
                f"Venda {i + 1}: Cliente: {venda['cliente']['nome']} - "
        ...
        while True:
            ...
            if event in (sg.WIN_CLOSED, "Cancelar"):
                ...
                if selected_index:
                    selected_index = int(selected_index[0].split(" ")[1].strip(":")) - 1
                    break
                else:
    ...
    def exibir_vendas(self, vendas: List[dict]):
        if not vendas:
            sg.popup("Não há vendas registradas.",
                     title="Aviso", text_color="white", background_color="#2C2F36")
            return
        ...
        for i, venda in enumerate(vendas, 1):
            try:
                ...
                data_venda = venda.get("data_venda", "Data não registrada")
                valor_total = venda.get("valor_total", 0.0)

                texto_formatado += f"Pedido {i} - Data: {data_venda}\n"
                texto_formatado += (f"• Cliente: {cliente.get('nome', 'N/A')}"
                                    f"(CPF: {cliente.get('cpf', 'N/A')})\n")
                texto_formatado += (f"• Vendedor: {vendedor.get('nome', 'N/A')}"
                                    f"(CPF: {vendedor.get('cpf', 'N/A')})\n")
                ...
                sg.popup(f"Erro ao processar vendas: campo '{e.args[0]}' não encontrado.",
                         title="Erro de Dados", text_color="red", background_color="#2C2F36")
                return
        ...
        layout = [
            ...
            [sg.Text("Detalhes das vendas estão listados abaixo:", text_color="white",
                     background_color="#1F1F1F", font=("Arial", 12))],
            [sg.Multiline(texto_formatado, size=(80, 25), background_color="#3E4349",
                          font=("Courier New", 12),
